...
def test_bm25_returns_relevant_doc_for_unique_query(synthetic_chunks, fresh_wr):
    ...
    assert score > 0
    assert top_chunk["file_id"] == "f002", (
        f"expected reward-hacking doc f002 to rank first, got {top_chunk['file_id']}"
    )
...
def test_bm25_explain_payload_present(synthetic_chunks, fresh_wr):
    ...
    hits = fresh_wr.bm25_search(
        "alignment", synthetic_chunks, k=3, explain=True
    )
    assert hits
...
def test_bm25_explain_does_not_poison_cache(synthetic_chunks, fresh_wr):
    ...
    assert not leaked, (
        f"explain payload leaked into source chunks: "
        f"{[c.get('file_id') for c in leaked]}"
    )
