...
def test_load_all_chunks_excludes_meta_files(real_index_dir, fresh_wr):
    ...
    leaked = [
        c for c in chunks
        if Path(c.get("relpath", "")).name in meta_basenames
    ]
    assert not leaked, (
        f"meta docs leaked into chunks: "
        f"{[c.get('relpath') for c in leaked[:5]]}"
    )
...
def test_audit_files_excluded_from_chunks(real_index_dir, fresh_wr):
    ...
    leaked = [
        c for c in chunks
        if Path(c.get("relpath", "")).name.startswith("_audit_")
    ]
    assert not leaked, (
        f"_audit_*.md leaked into chunks: "
        f"{[c.get('relpath') for c in leaked[:5]]}"
    )
...
def test_trash_dir_excluded_from_chunks(real_index_dir, fresh_wr):
    ...
    assert not leaked, (
        f"_trash content leaked: {[c.get('relpath') for c in leaked[:5]]}"
    )
